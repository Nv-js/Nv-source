    ...
    var modal={
        ...
        eventsFn:{
            ...
            modalMoveFn:function(opts,elem){
                var modals=$(elem).find(".nv-modal-content").get(0);
                if(opts.move){
                    $(elem).find(".nv-modal-content").addClass("nv-modal-content-move");
            ...
            modalFn:function(element,opts){
                ...
                $.each(dom,function(index,elem){
                    //console.log($(this).find(".nv-modal-title"))
                    ...
                    //关闭浮层调用
                    $(this).find(".nv-modal-title .nv-modal-close").off("click").on("click",modal.eventsFn.hideModalFn);
                    $(this).find(".nv-modal-footer .nv-modal-cancel").off("click").on("click",modal.eventsFn.hideModalFn);
                })
            ...
            middle:function(opts){
                if(opts.top!="" && (typeof opts.top=="number")){
                    $(opts.elements).find(".nv-modal-content").css({
                        "marginTop":opts.top+"px"
                    })
                    if(opts.move){
                        var width=$(opts.elements).find(".nv-modal-content").width();
                        $(opts.elements).find(".nv-modal-content").css({
                            "left":"50%",
                    ...
                    var height=$(opts.elements).find(".nv-modal-content").height(),
                        clientHeight=$(window).height()-$(opts.elements).find(".nv-modal-content").height();
                    if(opts.move){
                        var width=$(opts.elements).find(".nv-modal-content").width();
                        $(opts.elements).find(".nv-modal-content").css({
